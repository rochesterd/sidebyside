...
| `app_icon.py` | `icon_path()` — `assets/` beside the module in a checkout, `sys._MEIPASS/assets` in a frozen exe. Stdlib only, no Qt import, since `setup_wizard.py` is tkinter and needs the same path. Callers do `app.setWindowIcon(QIcon(str(icon_path(...))))`; a missing file gives a null `QIcon`, never an exception. |
