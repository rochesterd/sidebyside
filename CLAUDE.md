...
| `compositor.py` | `fit_into_canvas`, `side_by_side`, `picture_in_picture`, and `draw_timer` — all aspect-preserving, letterboxed into a fixed-size canvas. Called at *watch* time by `viewer.py` (and for the kiosk's live preview), not at record time. |
...
| `viewer.py` | `ViewerDialog` — playback for one session: play/pause, scrub, and a layout picker (side-by-side / PiP / either camera alone) applied live via `compositor.py`. A `QDialog` so `app.py` can open it modally *and* `main()` can run it standalone as `viewer.exe`. Student-facing; nothing here can modify or delete a recording. Tears down on `finished`, not `closeEvent` — it holds open PyAV decoders (see DECISIONS.md's Preview-leak entry). |
...
| `app.py` | The kiosk entry point (see CLAUDE.md "Who uses it"). Thin PySide6 shell: instrument picker, Start, Stop, and a Watch button that opens the just-finished session in `viewer.py` (modally; the live preview pauses, the cameras keep running). Picker disables once recording starts. Polls `KioskController` on a timer and reflects what it reports; owns no decisions itself. |
...
| `test_viewer.py` | Headless tests for `ViewerDialog` against a real recorded session (no `.show()`/`.exec()`): every layout mode renders, scrub pauses-then-resumes, playback stops at the end, and `reject()` (the Esc path) releases the PyAV decoders. |
