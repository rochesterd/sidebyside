...
| `exposure_calibration.py` | The pure maths (numpy only) behind `IdsCamera.auto_calibrate()`: metering modes (`METERING_HIGHLIGHT` for bright-beam-on-black frames), `exposure_budget_us()`, and `next_exposure_gain()`, which prefers exposure up to the budget and the least gain that will do. See DECISIONS.md's 2026-09-08 entries. |
| `device_presets.py` | The supported-camera registry: `DeviceProfile`/`PROFILES` (what settings.py offers and `config.json`'s `profile` resolves through), plus `orientation_for_model()`/`pixel_clock_hz_for_model()` reading that same table, so a preset and a picked profile can't disagree. `CUSTOM_PROFILE_ID` is deliberately absent from `PROFILES`: unknown degrades to custom, never to a failed load. Consulted by `IdsCamera._open()`; a `config.json` `orientation` overrides it. The older BIO's (no-op) orientation is defaulted in `net2860_winusb_camera.py` instead, since it has no IDS model string. Where this is headed: ROADMAP.md's device-profiles entry. |
...
| `net2860_winusb_camera.py` | `Net2860WinUsbCamera` — the older BIO's KS722OUP (EM2860 bridge), `kind: "net2860_winusb"`. The camera board runs its own AE/AWB, so there is no exposure/gain to offer; the bridge's picture registers (brightness, contrast, saturation, sharpness) *are* ours, and today replay Keeler's captured values — DECISIONS.md 2026-09-11, ROADMAP.md. `Frame.index` is the camera's own field counter, so its gaps are **real** drops. |
| `net2860_init.py` | `START_WRITES`/`STOP_WRITES` — the EM2860 init sequence captured from Keeler's driver and verified by register readback. Split in two at four writes that turned out to be picture registers, so the recorded reason (replaying it whole switches the bridge off) is unexplained; keep the split, which is what hardware verified. Its docstring says how to re-derive it; the capture is gitignored and irreplaceable. |
...
| `settings.py` | Technician tool: per role a device dropdown, a profile dropdown (instrument rows only; auto-selected from the model, Custom always available), an optional nickname, and Preview (with Auto-Calibrate), then Rescan, a recordings folder, opt-in retention, and Save to `config.json`. No hot reload. A separate program from `app.py`. |
